...
```
### calDecimal和baseDecimal：用于计算的基数
minAmount: 最小质押金额。
baseYear: 一年时间的常量。
PoolState: 池的状态枚举。
globalPaused：全局暂停标志。
swapRouter、feeAddress、oracle：合约地址。
lendFee、borrowFee：质押和借贷费用。
...
constructor(
    ...
    require(_swapRouter != address(0), "swapRouter address is zero");
    require(_feeAddress != address(0), "feeAddress address is zero");
...
- 初始化合约，设置Oracle、Swap Router 和 Fee address。
...
function setMinAmount(uint256 _minAmount) validCall external {
    require(_minAmount > 0, "minAmount is zero");
...
function depositBorrow(uint256 _pid, uint256 _stakeAmount) external payable nonReentrant notPause timeBefore(_pid) stateMatch(_pid) {_
    ...
    if (pool.borrowToken == address(0)) {
        ...
        _pool.borrowSupply = pool.borrowSupply.add(_stakeAmount);_
    }
...
function settle(uint256 _pid) public validCall {_
    ...
    if (pool.lendSupply > 0 && pool.borrowSupply > 0) {
        uint256[2] memory prices = getUnderlyingPriceView(_pid);_
        ...
        if (pool.lendSupply > actualValue) {
            ...
            data.settleAmountLend = pool.lendSupply;data.settleAmountBorrow = pool.lendSupply.mul(pool.martgageRate).div(prices[pool.martgageRate).div(prices[1].mul(baseDecimal).div(prices[0]));
        }
...
function finish(uint256 _pid) public validCall {_
    PoolBaseInfo storage pool = poolBaseInfo[_pid];_
    PoolDataInfo storage data = poolDataInfo[_pid];_
    require(block.timestamp > poolBaseInfo[_pid].endTime, "finish: less than end time");_
...
#### 清算
```javascript
function liquidate(uint256 _pid) public validCall {_
    PoolDataInfo storage data = poolDataInfo[_pid];_
    PoolBaseInfo storage pool = poolBaseInfo[_pid];_
    ...
    (uint256 amountSell, uint256 amountIn) = _sellExactAmount(swapRouter, token0, token1, sellAmount);_
    if (amountIn > lendAmount) {
        uint256 feeAmount = amountIn.sub(lendAmount);_
        redeem(feeAddress, pool.lendToken, feeAmount);_
    ...
    emit StateChange(_pid, uint256(PoolState.EXECUTION), uint256(PoolState.LIQUIDATION));_
}
```
